 ...
 * (cover-crop) + a rounded clip-path. The browser/editor honors both, but
 * standalone SVG renderers disagree: Word/Office drop a clipped image whose
 * clipPath sits later in the document (the right photo vanishes), and Figma
 * skews images when clip-path/transform is involved. There is no single SVG
 * attribute combination that renders correctly everywhere.
 *
 * So for export we pre-render each such image onto a canvas at the box size —
 * cover-cropped, with the rounded corners cut out as transparency — and replace
 * the <image> with that PNG placed plainly (no clip-path, no preserveAspectRatio,
 * no transform). A plain positioned image renders identically in every tool.
 */
...
export async function bakeImages(svg: SVGSVGElement): Promise<void> {
  ...
  for (const el of images) {
    ...
    // Only images that depend on clip-path or slice need baking; plain ones are fine.
