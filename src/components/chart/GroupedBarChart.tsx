    ...
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      >
        ...
        {categories.map((cat, ci) => {
          ...
          // Grouped bars — each series bar starts from zero, stacked vertically
          const zeroX = xScale(Math.max(0, minVal));
          const barElements: React.ReactNode[] = [];
          const labelElements: React.ReactNode[] = [];

          const numSeries = series.length;
          const groupSpacing = settings.bars.spacingInStack;
          const subBarH = numSeries > 1
            ? Math.max(2, (barHeight - groupSpacing * (numSeries - 1)) / numSeries)
            : barHeight;
          ...
          series.forEach((s, si) => {
            ...
            // Data point labels
            if (settings.labels.showDataPointLabels && (rawValue !== 0 || settings.labels.showZeroValues)) {
              ...
              const labelPos = settings.labels.dataPointPosition === 'custom'
                ? (settings.labels.dataPointCustomMode === 'row'
                  ? (settings.labels.dataPointRowPositions?.[cat]?.[s.key] || settings.labels.dataPointRowPositions?.[cat]?.['__default'] || 'center')
