...
export function ShareVisualizationDialog({
  ...
  const handleShare = async () => {
    ...
    if (!hasSelection || (vizIds.length === 0 && folderIds.length === 0)) return;
    ...
    try {
      ...
      toast.success(
        `Shared ${totalItems} item${totalItems > 1 ? 's' : ''} with ${recipientCount} recipient${recipientCount > 1 ? 's' : ''}`
      );
      onShared?.();
      onOpenChange(false);
    } catch {
  ...
  const titleText = (() => {
    const total = vizIds.length + folderIds.length;
    if (folderIds.length > 0 && vizIds.length > 0) return `${total} items`;
    if (folderIds.length > 0) return folderIds.length > 1 ? `${folderIds.length} folders` : 'folder';
    return vizIds.length > 1 ? `${vizIds.length} visualizations` : 'visualization';
  })();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        {isAdmin ? (
          // ── Admin: user list with checkboxes ──
          loading ? (
            ...
            <div className="space-y-3 py-2">
              <p className="text-xs text-gray-500">
                Select users to share with (copies will be created for each user):
              </p>
